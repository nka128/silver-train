...
- **[system_files/usr/bin/nordvpn-group](./system_files/usr/bin/nordvpn-group)** — Ensures the **`nordvpn` group** exists (**`groupadd`** when missing, per NordVPN), then **`usermod -aG nordvpn`** for each **`wheel`** member. **No version stamp or early exit**: the job runs whenever the unit starts (e.g. each boot), so **admins added to `wheel` later** still get the group. **`ensure_group`** and **`usermod -aG`** are idempotent; rationale is in the script header.
- **[system_files/usr/lib/systemd/system/nordvpn-group.service](./system_files/usr/lib/systemd/system/nordvpn-group.service)** — **`Type=oneshot`**, **`RemainAfterExit=yes`**, **`DefaultDependencies=no`**, **`After=local-fs.target`**, **`Before=nordvpnd.socket`** / **`nordvpnd.service`**, **`WantedBy=sockets.target`**, **`ExecStart=/usr/bin/nordvpn-group`**.
- **[system_files/usr/lib/systemd/system/nordvpnd.socket.d/10-nordvpn-group.conf](./system_files/usr/lib/systemd/system/nordvpnd.socket.d/10-nordvpn-group.conf)** — Drop-in so **`nordvpnd.socket` is ordered after and requires `nordvpn-group.service`**, so the **`nordvpn` group exists before systemd resolves it for the socket.
